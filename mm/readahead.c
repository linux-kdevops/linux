		...
		unsigned long nr_to_read, unsigned long lookahead_size)
{
	...
	unsigned long i = 0;
	unsigned int min_pages = (1UL << mapping_min_folio_order(mapping));
	...
	while (i < nr_to_read) {
		...
		if (folio && !xa_is_value(folio)) {
			...
			VM_BUG_ON_FOLIO(nr_pages < min_pages, folio);
		...
		unsigned long nr_to_read)
{
	...
	unsigned int min_pages = (1U << min_order);
	...
	if (min_order && (index & (min_pages - 1))) {
		unsigned long old_index = index;

		index = round_down(index, min_pages);
